...
namespace NPCMake.Core.Utils.Tinifan.Binary.Logic
{
    public class Entry
    {
        ...
        public string[] GetDistinctStrings(List<Entry> entries = null)
        {
            ...
            distinctStrings.AddRange(Variables.Where(x => x.Type == Type.String).Select(x => Convert.ToString(x.Value)).Distinct().ToArray());
        ...
        {
            foreach (Variable variable in Variables.Where(x => x.Type == Type.String))
            {
