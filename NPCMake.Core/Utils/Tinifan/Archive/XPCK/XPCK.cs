...
using NPCMake.Core.Utils.Tinifan.Compression.LZ10;
using NPCMake.Core.Utils.Tinifan.Archive;
using NPCMake.Core.Utils.Tinifan.Compression;
using NPCMake.Core.Utils.Tinifan.Tools;

namespace NPCMake.Core.Utils.Tinifan.Archive.XPCK
{
    public class XPCK : IArchive
    {
        ...
        public void Save(string fileName)
        {
            using (FileStream stream = new FileStream(fileName, FileMode.Create, FileAccess.Write))
            {
                ...
                byte[] tableNameData = CompressBlockTo(tableName.SelectMany(bytes => bytes).ToArray(), new LZ10());
        ...
        private byte[] SerializeData<T>(T[] data)
        {
            ...
            BinaryDataWriter writer = new BinaryDataWriter(stream);
            writer.WriteMultipleStruct(data);
            writer.Dispose();
